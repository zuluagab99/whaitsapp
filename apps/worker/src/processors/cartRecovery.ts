...
/**
 * Delayed cart-recovery job. Re-checks state at execution time (checkout may
 * have completed meanwhile), enforces opt-in, then sends the merchant's
 * approved marketing template through the standard outbound pipeline.
 */
export async function processCartRecovery(ctx: WorkerContext, job: CartRecoveryJob): Promise<void> {
  const send = await tenantTransaction(ctx.db, job.tenantId, async (tx) => {
    ...
    // The merchant's approved recovery template (Meta-approved marketing category).
